...
async def solar_range(req: RangeRequest) -> Dict[str, Any]:
    try:
        ...
        # flags: Swiss ephemeris, speed not required here
        ifl = swe.FLG_SWIEPH
        while True:
            retflag, tret, attr, serr = swe.sol_eclipse_when_glob(jd, ifl, 0)
            if retflag == 0:
                break
            jd_max = float(tret[0])
            if jd_max > jd_end:
            ...
            count_guards += 1
            if count_guards >= req.max_events:
                break
            # sonraki tutulmaya atla (1 gün ileri güvenli)
            jd = jd_max + 1.0
...
async def lunar_range(req: RangeRequest) -> Dict[str, Any]:
    try:
        ...
        ifl = swe.FLG_SWIEPH
        while True:
            retflag, tret, attr, serr = swe.lun_eclipse_when_glob(jd, ifl, 0)
            if retflag == 0:
                break
            jd_max = float(tret[0])
            if jd_max > jd_end:
